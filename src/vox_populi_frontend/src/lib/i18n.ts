...
survey: {
    ...
    results: {
      ...
      securityMatrix: "Confianza (Preguntas 8-10)",
    ...
    audit: {
      ...
      moduleHashNote: "Nota técnica: Este valor corresponde al module_hash del canister frontend en ICP (hash SHA-256 del módulo Wasm actualmente instalado en ese canister). Describe la huella criptográfica del ejecutable WebAssembly del asset canister, no la de los recursos web individuales. La integridad de HTML/CSS/JS se valida por separado contra el árbol certificado del asset canister: verify.sh contrasta cada archivo con su entrada on-chain (encoding identity) y su digest SHA-256 asociado.",
      ...
      verify: {
        ...
        step1Desc: "Necesitas Git (>= 2.40), GitHub CLI (>= 2.80), jq (>= 1.6), tar (>= 3.5) y una CLI de ICP (icp >= 0.2 recomendado; dfx >= 0.31 como fallback). Verificado oficialmente en Linux/macOS. La variante PowerShell se incluye como referencia y no esta probada oficialmente.",
        step1Command: "# Herramientas minimas (versiones recomendadas)\ngit --version  # >= 2.40\ngh --version   # >= 2.80\njq --version   # >= 1.6\ntar --version  # >= 3.5\n\n# CLI ICP (preferida: >= 0.2) o DFX (fallback: >= 0.31)\nicp --version || dfx --version\n\n# Autenticacion GitHub para descargar artefactos\ngh auth status",
...
survey: {
    ...
    results: {
      ...
      securityMatrix: "Trust (Questions 8-10)",
    ...
    audit: {
      ...
      moduleHashNote: "Technical note: This value is the frontend canister module_hash on ICP (the SHA-256 hash of the Wasm module currently installed in that canister). It fingerprints the WebAssembly executable of the asset canister, not the individual web resources. HTML/CSS/JS integrity is verified separately against the certified asset-canister state: verify.sh checks each file against its on-chain entry (identity encoding) and associated SHA-256 digest.",
      ...
      verify: {
        ...
        step1Desc: "You need Git (>= 2.40), GitHub CLI (>= 2.80), jq (>= 1.6), tar (>= 3.5), and an ICP CLI (icp >= 0.2 recommended; dfx >= 0.31 fallback). Officially tested on Linux/macOS. PowerShell commands are included as reference and are not officially tested.",
        step1Command: "# Minimum tooling (recommended versions)\ngit --version  # >= 2.40\ngh --version   # >= 2.80\njq --version   # >= 1.6\ntar --version  # >= 3.5\n\n# ICP CLI (preferred: >= 0.2) or DFX (fallback: >= 0.31)\nicp --version || dfx --version\n\n# GitHub auth for downloading release artifacts\ngh auth status",
...
survey: {
    ...
    results: {
      ...
      securityMatrix: "Confiança (Preguntes 8-10)",
    ...
    audit: {
      ...
      moduleHashNote: "Nota tècnica: Aquest valor correspon al module_hash del canister frontend a ICP (hash SHA-256 del mòdul Wasm actualment instal·lat en aquest canister). Defineix la petjada criptogràfica de l'executable WebAssembly de l'asset canister, no la dels recursos web individuals. La integritat d'HTML/CSS/JS es valida per separat contra l'estat certificat de l'asset canister: verify.sh contrasta cada fitxer amb la seva entrada on-chain (encoding identity) i el seu digest SHA-256 associat.",
      ...
      verify: {
        ...
        step1Desc: "Necessites Git (>= 2.40), GitHub CLI (>= 2.80), jq (>= 1.6), tar (>= 3.5) i una CLI d'ICP (icp >= 0.2 recomanat; dfx >= 0.31 com a fallback). Provat oficialment a Linux/macOS. Els comandos de PowerShell s'inclouen com a referencia i no estan provats oficialment.",
        step1Command: "# Eines minimes (versions recomanades)\ngit --version  # >= 2.40\ngh --version   # >= 2.80\njq --version   # >= 1.6\ntar --version  # >= 3.5\n\n# CLI ICP (recomanada: >= 0.2) o DFX (fallback: >= 0.31)\nicp --version || dfx --version\n\n# Autenticacio GitHub per descarregar artefactes\ngh auth status",
