...
function ChartTooltipContent({
  ...
  labelKey,
  ...props
}: any) { // Usamos any aquí para evitar el error de Omit/Props en las líneas 109/114
  ...
  return (
    <div className={cn('flex items-center justify-center gap-4', verticalAlign === 'top' ? 'pb-3' : 'pt-3', className)}>
      {payload.map((item: any, index: number) => {
        ...
        const itemConfig = getPayloadConfigFromPayload(config, item, key)
        return (
          <div key={item.value || index} className="[&>svg]:text-muted-foreground flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3">
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: any, key: string) {
  ...
  const payloadPayload = payload && 'payload' in payload && typeof payload.payload === 'object' && payload.payload !== null ? payload.payload : undefined
  let configLabelKey: string = key
  if (payload && key in payload && typeof (payload as any)[key] === 'string') {
