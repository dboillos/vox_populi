#!/bin/bash
# VERSION: 1.2.54-TFM-TOTAL-INTEGRITY
# OBJETIVO: Match 100% en Backend (WASM) y Frontend (Descarga Real de Red).

if [ -z "$1" ]; then
    echo -e "\033[0;31mError: Proporciona un tag (ej: v1.2.54)\033[0m"
    exit 1
fi
...
echo -e "\n\033[0;34m[2/4] Fase 2: Despliegue en Mainnet (IC)...\033[0m"
...
if [ -d "$FRONTEND_DIST" ]; then
    cd "$FRONTEND_DIST"
    # Generamos el manifiesto local: Hash SHA256 del contenido real
    find . -type f ! -name "*.map" ! -name "*.json5" ! -name "*.json" | sort | while read -r file; do
        hash=$(sha256sum "$file" | awk '{print $1}')
        echo "$hash ${file#./}"
    done > ../../../assets.manifest
    cd ../../..
    # Firma global local para el registro de Git
    sha256sum assets.manifest | awk '{print $1}' > assets.hash
else
    echo -e "\033[0;31mError: Assets no encontrados.\033[0m"; exit 1
fi

echo -e "\n\033[0;34m[4/4] Fase 4: Registro de Evidencias en Git...\033[0m"
git add .
git commit -m "release: $NEW_TAG (Total Integrity Certified)"
git tag -a "$NEW_TAG" -m "$NEW_TAG"
git push origin main --tags --force

# ==============================================================================
#             AUDITORÍA DE INTEGRIDAD DEL BACKEND (RESTAURADA)
# ==============================================================================
echo -e "\n\033[1;35m===============================================================\033[0m"
...
# ==============================================================================
#             AUDITORÍA DE INTEGRIDAD DEL FRONTEND (VÍA DESCARGA)
# ==============================================================================
echo -e "\n\033[1;36m===============================================================\033[0m"
echo -e "\033[1;36m             AUDITORÍA DE INTEGRIDAD DEL FRONTEND            \033[0m"
echo -e "\033[1;36m===============================================================\033[0m"
echo "Ejecutando descarga y verificación de activos desde la red..."

# Llamada al script auxiliar que descarga y calcula hashes reales
...
if [ "$LOCAL_SIG" == "$NET_ROOT_HASH" ]; then
    ...
    echo -e "Nota: Si los hashes difieren tras la descarga, el build local no coincide con la red."
fi
echo -e "\033[1;36m===============================================================\033[0m\n"